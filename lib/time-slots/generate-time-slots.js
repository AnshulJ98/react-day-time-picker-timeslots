...
function generateTimeSlots(selectedDate, slotSizeMinutes) {
  ...
  start = _dateFns["default"].addHours(start, 8);

  if (isToday) {
    var now = new Date();

    var offsetHours = _dateFns["default"].getHours(now); // "Pad" the start time with the amount of hours of the current time, to
